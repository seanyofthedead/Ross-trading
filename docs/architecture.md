...
**Structure (how the six setups below are built):** each setup is a pure-sync `detect(snapshot) -> Optional[EntrySignal]` atom behind a thin registry; the as-of snapshot is widened to carry a *completed-bar* intraday series and the live quote, and detectors *recompute* indicators from that series each tick. See "Resolved Decisions" appendix → D6 for the full rationale, the rejected alternatives, and the determinism/evaluability contract every detector below must honor.
...
* **D6 / Pattern Detector structure — “A′”: pure per-pattern detectors over a widened as-of snapshot; assembler stays the sole determinism boundary.** Decided via a three-persona architecture debate (pragmatist / purist / quant-skeptic, two rounds + a code spike). All six §3.4 setups are added incrementally as small atoms with one shared plumbing change, emitting one uniform `EntrySignal`, without forking the scanner and while preserving bit-identical live/replay determinism.

  **Decision.** Each setup is a pure-sync atom `detect(snapshot) -> Optional[EntrySignal]` dispatched by a thin registry (mirrors `scanner/strength.py`’s “pure consumer of a cache” shape). The as-of snapshot is **widened** to carry a *completed-bar intraday series* plus the live quote; detectors **recompute** indicators (ATR(14,1-min), VWAP, intraday 9-EMA, pre-market high, pole/flag/flat-top geometry) from that series on every tick. The `SnapshotAssembler` remains the **single** replay-determinism boundary; recompute over the as-of series is bit-identical by construction because every indicator is a pure reduction over the series, not a hidden-state recurrence. No new stateful component is introduced.

  **Rejected.** *Option B (stateful tick-by-tick `PatternDetector`):* unanimously rejected — it moves mutable rolling state into the money path *outside* the assembler boundary and re-couples detection to I/O arrival order, which replay provably does not reproduce (`scanner/replay.py::_load_assembler` drains M1, then D1, then quotes separately; `data/providers/replay.py` does zero pacing in `AS_FAST_AS_POSSIBLE`). *Literal Option A (“precompute intraday indicators in the analytic cache like `daily_emas`”):* not achievable — the spike confirmed `data/cache.py` holds only daily tables (`daily_volumes`, `daily_emas`, `daily_bars`, keyed by date) and `scanner/types.py::ScannerSnapshot` carries only the *last completed bar*, no intraday series; there is nothing to precompute into and nothing to recompute from until the snapshot is widened. *Option C/D (stateful rolling-window builder / immutable fold):* deferred — a memoizing builder can only ever *equal* the recompute (that equality is exactly what its `builder == recompute` parity test would assert), so it is a profiling-gated optimization, not a design commitment. Recompute is the determinism **oracle**; if a builder ever ships, this same recompute path is its test oracle for free. At Phase-2 scale (2–3 watchlist names, ~2 s ticks, a few hundred Decimal bars) recompute cost is negligible — YAGNI until a profiler, not a hunch, says otherwise.

  **Snapshot shape (load-bearing — NOT a uniform “last-N”).** VWAP and the intraday 9-EMA are **session-anchored cumulative** reductions, so the 1-min series must run from a *fixed origin*, not a sliding tail (a tail would resurrect a seed-divergence bug between a mid-session live attach and a from-open replay): **1-min = session-to-date from the fixed 04:00 ET anchor** (≈240–300 bars/name; detectors slice `[04:00,09:30]` for pre-market high, `[09:30,anchor]` for VWAP/9-EMA seed, last ~15 for ATR(14), last 1–3 for micro-pullback); **5-min = last ~30 bars**; **10-sec = last ~60 bars**. The live side retains session-to-date 1-min bars in RAM (bounded, raw observed data — *not* derived indicator state), so the determinism boundary holds.

  **Path-independent contract (pin into §3.4 regardless; these are money-path correctness rules, not A′-vs-builder choices):**
  1. **Completed bars only.** The intraday series feeds detectors *only* finalized bars. Add an explicit `is_final` flag on `Bar` (`data/types.py`) as a producer-side invariant/assert so a forming candle can never enter ATR/VWAP/EMA — `Bar.ts` is open time and today “completed” is enforced only by the assembler’s `_last_at_or_before` selector, not by the type.
  2. **Cross-triggers read the quote `last`, never a forming bar’s running high.** Bar-statistic predicates (close-above-trendline, ATR) use completed bars; “price crosses above pre-market/flag high” uses the live quote already in the snapshot. Indicator values are identical across paths via (1); the *trigger* is identical only if sourced from `last`. `EntrySignal` declares its `trigger_source` per pattern.
  3. **Warm-up = `None`, and `None` ≠ fire.** Indicators are `None` until seeded; detectors treat `None` as “do not fire” (the `strength.py` “absence of evidence is not promotion” rule), never a permissive default.
  4. **Feed gaps are explicit, not bare holes.** The as-of series must distinguish “no bar because no trade” from “no bar because feed gap”: the assembler surfaces the recorded `FeedGap` (already captured/replayed via `scanner/replay.py::subscribe_feed_gaps`; already first-class in `loop.py::on_feed_gap`) as a **gap sentinel** spanning the affected interval, which detectors refuse to bridge — otherwise a flat-top/micro-pullback computed across a gap connects candles over missing time and emits a confident-but-false pattern.
  5. **As-of ordering is `(exchange_ts, seq)`, not bare `ts`; halts and corrections are typed (Wave 0 ingestion contract).** `Quote`/`Bar`/`Tape` carry a per-`(symbol, channel)` monotonic vendor `seq` and a three-way timestamp split (`exchange_ts`/`vendor_ts`/`ingest_ts`, `data/types.py`). The assembler and `scanner/replay.py` order as-of selection strictly on `(exchange_ts, seq)` and dedup on the scoped `(symbol, channel, seq)` key, so identical inputs assemble to identical snapshots regardless of arrival order; **staleness keys on `ingest_ts`** (local receipt vs wall-clock), distinct from the as-of `exchange_ts`. Silent vendor drops surface as a `FeedGap` from **seq discontinuity** (`data/reconnect.py`), not only on socket disconnect. A venue **`Halt`/resume** is a typed event distinct from a gap — the assembler omits a halted symbol and refuses to price `last` off a pre-halt quote after a resume — and a **`Correction`/bust** is an append-only delta that deterministically re-folds the amended print’s volume into its covering bar. The recording codec is `SCHEMA_VERSION = 2` with every v1 decoder retained (synthesized defaults: `seq` from file order, `exchange_ts = vendor_ts = ts`, `ingest_ts = ts_recorded`), so v1 recordings replay bit-identically under the v2 build.

  **`EntrySignal` value object.** `frozen=True, slots=True`, mirroring `ScannerPick`. Fields: `pattern_id`, `timeframe`, `entry_price`, `stop_price` (so `stop_distance` and the §3.5 sizer’s `shares = max_risk / stop_distance` reconstruct), `avg_1min_volume` (the §6 capacity-cap **denominator** — §6 caps shares at 1–2 % of the symbol’s *average 1-minute volume*, architecture.md:444, **not** a single bar — so the sizer can enforce the cap; capacity realism lives nowhere else), `breakout_bar_volume` (the trigger bar’s own volume, for the per-setup volume-confirmation predicate, e.g. “volume > avg_5min_volume” / “≥ pole avg”), `trigger_price`/`trigger_source`, `stop_basis`, the indicator evidence used, and `anchor_ts`/`bar_ts`. No-fire is `Optional[EntrySignal] = None`, never a falsy signal. The `pattern_id` registry is pinned to a `Literal` **mirrored into §3.4** (the way `RejectionReasonLit` mirrors `RejectionReason` with `assert_never` exhaustiveness in `loop.py::_lit_to_enum`), so a seventh setup is a compile-time-visible, Drift-CI-catchable contract change.

  **Known limitation — per-pattern evaluability is not wired yet (cross-repo blind spot).** The journal will log `pattern_id`, but the ground-truth corpus (`ross-trading-research/pipeline/extract/emit.py`) currently emits only `{ticker, direction, time_called_out, notes}` with **no setup label**, and matching is ticker-only — so per-pattern precision/recall is *unmeasurable today*. Pin the `pattern_id` `Literal` now so journal rows join cleanly the day the corpus gains a `setup` field; until a *shared, versioned schema artifact* spans the two repos, this alignment (and the related `direction "short"` vs case-strict `"long"` and committed-`cookies.txt` issues) is a drift point Drift CI cannot see.

  **Build order (one atom = one plan file, mypy --strict + ruff + pytest).** (1) `EntrySignal` + the `pattern_id` `Literal`; (2) the snapshot-widening + `is_final` + gap-sentinel on the assembler/`Bar`/`ScannerSnapshot`; (3) the missing pure Decimal indicator primitives (`indicators/atr.py`, `indicators/vwap.py`; EMA exists); (4) ship **Gap-and-Go pre-market-high break** first, journal it, then add the remaining five detectors one PR at a time. Code anchors: `src/ross_trading/scanner/strength.py` (atom template), `src/ross_trading/scanner/assembler.py` + `scanner/replay.py` (the boundary to widen), `src/ross_trading/scanner/types.py` (`ScannerSnapshot`/`ScannerPick` — structural template only), `src/ross_trading/patterns/types.py` (`EntrySignal` — **new** package; `EntrySignal` does **not** live in `scanner/types.py`, per the A1 plan), `src/ross_trading/data/types.py` (`Bar.is_final`), `src/ross_trading/indicators/`.
