...
* **D6 / Pattern Detector structure — “A′”: pure per-pattern detectors over a widened as-of snapshot; assembler stays the sole determinism boundary.** Decided via a three-persona architecture debate (pragmatist / purist / quant-skeptic, two rounds + a code spike). All six §3.4 setups are added incrementally as small atoms with one shared plumbing change, emitting one uniform `EntrySignal`, without forking the scanner and while preserving bit-identical live/replay determinism.
  ...
  **`EntrySignal` value object.** `frozen=True, slots=True`, mirroring `ScannerPick`. Fields: `pattern_id`, `timeframe`, `entry_price`, `stop_price` (so `stop_distance` and the §3.5 sizer’s `shares = max_risk / stop_distance` reconstruct), `breakout_bar_volume` (so the §6 capacity cap, ≤1–2 % of avg 1-min volume — architecture line ~442 — is enforceable; capacity realism lives nowhere else), `trigger_price`/`trigger_source`, `stop_basis`, the indicator evidence used, and `anchor_ts`/`bar_ts`. No-fire is `Optional[EntrySignal] = None`, never a falsy signal. The `pattern_id` registry is pinned to a `Literal` **mirrored into §3.4** (the way `RejectionReasonLit` mirrors `RejectionReason` with `assert_never` exhaustiveness in `loop.py::_lit_to_enum`), so a seventh setup is a compile-time-visible, Drift-CI-catchable contract change.
  ...
  **Build order (one atom = one plan file, mypy --strict + ruff + pytest).** (1) `EntrySignal` + the `pattern_id` `Literal`; (2) the snapshot-widening + `is_final` + gap-sentinel on the assembler/`Bar`/`ScannerSnapshot`; (3) the missing pure Decimal indicator primitives (`indicators/atr.py`, `indicators/vwap.py`; EMA exists); (4) ship **Gap-and-Go pre-market-high break** first, journal it, then add the remaining five detectors one PR at a time. Code anchors: `src/ross_trading/scanner/strength.py` (atom template), `src/ross_trading/scanner/assembler.py` + `scanner/replay.py` (the boundary to widen), `src/ross_trading/scanner/types.py` (`ScannerSnapshot`/`EntrySignal`), `src/ross_trading/data/types.py` (`Bar.is_final`), `src/ross_trading/indicators/`.
